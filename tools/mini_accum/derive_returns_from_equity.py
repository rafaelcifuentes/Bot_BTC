...
def parse_window_from_name(base: str) -> str | None:
    hits = RE_WINDOW_ANY.findall(base)
    if not hits: return None
    last = hits[-1]
    return f"WF_{last}"

def parse_config_from_name(base: str) -> str | None:
    def g(rx):
        m = rx.search(base)
...
def detect_equity_col(df: pd.DataFrame, override: str | None) -> str | None:
