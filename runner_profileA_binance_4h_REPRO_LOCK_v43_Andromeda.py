# runner_profileA_binance_4h_REPRO_LOCK_v43_Andromeda.py
# 4h swing — ccxt+fallbacks loader, ML prob_up, ATR risk, WF + HOLDOUT
# Repro flags: --primary-only, --freeze-end, --date-split, --repro-lock
...
# Autolock policy (habilitado por defecto; el flag --repro-lock lo bloquea)
...
# -----------------------
def parse_args():
    ap = argparse.ArgumentParser(description="Runner Andromeda (reproducible)")
    ap.add_argument("--primary-only", action="store_true",
                    help="Usar solo Binance como fuente OHLCV")
    ap.add_argument("--freeze-end", type=str, default=None,
                    help='Cortar datos hasta esta fecha/hora UTC, ej: "2025-08-10 04:00"')
    ap.add_argument("--date-split", type=str, default=None,
                    help='Split train/test por fecha (YYYY-MM-DD)')
    ap.add_argument("--repro-lock", action="store_true",
                    help="No modificar lockfile y restaurarlo si cambia")
...
def fetch_yf(symbol: str, period_days: int = 720) -> pd.DataFrame:
    days = max(365, min(720, period_days))  # 4h cap ~730d
    if yf is None:
        return pd.DataFrame()
    try:
...
def split_by_date(df: pd.DataFrame, date_str: str):
    ts = pd.Timestamp(date_str, tz="UTC").tz_convert(None)
    return df.loc[:ts].copy(), df.loc[ts:].copy()
...
# -----------------------
# Micro-grid (alrededor de los params actuales)
# -----------------------
...
def main():
    ...
    # Congelar dataset por freeze_end (si se pasa)
    if args.freeze_end:
    ...
    # Split por fecha si se pide, si no 70/30
    if args.date_split:
        train, test = split_by_date(df, args.date_split)
        if len(train) < 200 or len(test) < 50:
            print("Date split deja muy pocos datos; usando 70/30 por seguridad.")
    ...
    if not wf.empty:
        ...
        print(f"\nWF agg: mean net {wf_agg['mean_net']:.2f}, median PF {wf_agg['median_pf']:.2f}, "
              f"mean WR {wf_agg['mean_wr']:.2f}%, trades {wf_agg['total_trades']}, "
              f"median MDD {wf_agg['median_mdd']:.2f}, mean score {wf_agg['mean_score']:.2f}, folds {wf_agg['folds']}")
    else:
        wf_agg = {}

    print(f"\n— Holdout (last {HOLDOUT_DAYS} days) —")
    ...
    # Stamp coherente (si hay freeze_end, úsalo para el nombre)
    stamp = (pd.Timestamp(args.freeze_end, tz="UTC").strftime("%Y%m%d_%H%M")
             if args.freeze_end else now_stamp())
    ...
    # Repro lock: NO cambios permanentes al lock (y restaurar si algo lo tocó)
    if args.repro_lock:
        if os.path.exists(LOCK_PATH):
            bak = LOCK_PATH + ".repro_bak"
            # si no existe backup, créalo ANTES
            if not os.path.exists(bak):
                with open(LOCK_PATH, "rb") as fsrc, open(bak, "wb") as fdst:
                    fdst.write(fsrc.read())
            # si cambió, restáuralo
            with open(LOCK_PATH, "rb") as fnow, open(bak, "rb") as fbak:
                if fnow.read() != fbak.read():
                    print("↩︎ Repro-lock: lockfile cambió; restaurando versión previa…")
                    with open(bak, "rb") as fbak2, open(LOCK_PATH, "wb") as fout:
                        fout.write(fbak2.read())
        print("↩︎ Autolock deshabilitado por --repro-lock.")
        return

    # Autolock (si no se pidió modo repro)
