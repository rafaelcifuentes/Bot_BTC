...
def ensure_index_tzaware(df: pd.DataFrame | None) -> pd.DataFrame | None:
    if df is None:
        return None
...
def read_csv_tzaware(path: str | os.PathLike[str]) -> pd.DataFrame:
    p = os.fspath(path)  # asegura str para pandas/os
    if not os.path.exists(p):
        raise FileNotFoundError(p)

    df = pd.read_csv(p)

    # Asegura índice datetime (UTC) si el CSV trae columna temporal
    if not isinstance(df.index, pd.DatetimeIndex):
        # intenta 'timestamp' o primera columna como fecha
        time_col = None
        lc = {c.lower(): c for c in df.columns}
        if 'timestamp' in lc:
            time_col = lc['timestamp']
...
def load_signals_from_root(root, wlabel: str, asset: str) -> tuple[pd.DataFrame | None, str | None]:
    """
    Intenta cargar señales desde `root/<wlabel>/<asset>.csv`.
    Si root es None/"none"/"auto" o no existe el archivo, devuelve (None, None).
    """
    if root is None:
        return None, None
    root_str = str(root).strip()
    if root_str == "" or root_str.lower() in ("none", "auto"):
        return None, None

    path = Path(root_str) / wlabel / f"{asset}.csv"
    if not path.exists():
        return None, None
...
def pick_signal_column(df: pd.DataFrame):
    if df is None:
        raise ValueError("[signals] DataFrame es None (no hay señales cargadas)")
...
def main() -> None:
    ...
    for asset in args.assets:
        ...
        for (wlabel, wstart, wend) in iter_windows_generic(windows):
            ...
            sig_df = ensure_index_tzaware(sig_df) if sig_df is not None else None

            if sig_df is None:
                print(f"[skip] no signals -> {wlabel}/{asset}  (signals_root={args.signals_root})")
                # saltamos este asset/window para evitar crash
                continue
