# runner_conflict_guard_weekly_v1.py
# Weekly runner — Andromeda Conflict-Guard (LONG+SHORT) con autolock de pesos si mejora Score y MDD dentro de cap.
# Usa los locks de:
#   LONG : ./profiles/profileA_binance_4h.json
#   SHORT: ./profiles/profileA_short_binance_4h.json
# Mantiene/actualiza pesos en:
#   ./profiles/conflict_guard_weights.json
...
WEIGHTS_FILE = "./profiles/conflict_guard_weights.json"
...
LATEST_SUMMARY = "./reports/conflict_guard_weekly_latest.json"
    ...
    for sym in symbols:
        ...
        while True:
            ...
            if len(all_rows) >= MAX_BARS + 1200:
                break
            try:
                time.sleep((ex.rateLimit or 200) / 1000.0)  # rate limit
            except Exception:
                pass
        if all_rows:
...
def load_weights(default_wL=0.60, default_wS=0.40) -> Tuple[float, float]:
    if os.path.exists(WEIGHTS_FILE):
        try:
            with open(WEIGHTS_FILE, "r") as f:
                o = json.load(f)
                return float(o.get("wL", default_wL)), float(o.get("wS", default_wS))
        except Exception:
            pass
    return default_wL, default_wS

def save_weights(wL: float, wS: float):
    payload = {
        "wL": float(wL), "wS": float(wS),
        "_meta": {"timestamp_utc": pd.Timestamp.utcnow().isoformat(), "note": "weekly autolock"}
    }
...
def run_portfolio(df: pd.DataFrame, pL: Dict, pS: Dict, wL: float, wS: float, cost=BASE_COST, slip=BASE_SLIP) -> Dict:
    # Los pesos escalan el risk_perc (no el ATR)
    pL = pL.copy(); pS = pS.copy()
    pL["risk_perc"] *= float(wL)
    pS["risk_perc"] *= float(wS)

    equity_closed = 10000.0  # referencia base para métricas relativas
    trades = []
    eq_curve = []
    pos = None
    ...
    for _, r in df.iterrows():
        ...
        # conflict-guard (no abrir si hay posición)
        if pos is None:
            ok_long = (
...
# -----------------------
def parse_args():
    ap = argparse.ArgumentParser(description="Weekly Andromeda Conflict-Guard Runner")
    ap.add_argument("--freeze-end", type=str, default=None,
                    help='Corta datos hasta esta fecha/hora UTC, ej: "2025-08-10 04:00"')
    ap.add_argument("--oos-days", type=int, default=90,
                    help="Ventana OOS para el reporte semanal (por defecto 90 días)")
    ap.add_argument("--train-frac", type=float, default=0.7,
                    help="Fracción de datos para entrenar el modelo prob_up si no se usa ventana OOS (fallback)")
    ap.add_argument("--wL", type=float, default=None, help="Peso LONG (si se pasa, sobrescribe el lock de pesos)")
    ap.add_argument("--wS", type=float, default=None, help="Peso SHORT (si se pasa, sobrescribe el lock de pesos)")
    ap.add_argument("--mdd_cap_mode", type=str, default="min_leg*1.05",
                    help='Cap de MDD: "min_leg*1.05", "max_leg", o un número fijo (ej "120")')
    ap.add_argument("--autolock", action="store_true",
                    help="Si mejora score y MDD ≤ cap, guardar nuevos pesos en profiles/conflict_guard_weights.json")
    return ap.parse_args()

def parse_mdd_cap(mode: str, rL: Dict, rS: Dict) -> float:
    mode = str(mode).strip().lower()
    if mode == "min_leg*1.05":
        base = min(rL["mdd"], rS["mdd"])
        return float(base * 1.05)
    if mode == "max_leg":
        return float(max(rL["mdd"], rS["mdd"]))
    try:
        return float(mode)
    except Exception:
        return float(min(rL["mdd"], rS["mdd"]) * 1.05)
...
def main():
    ...
    # Split: entrenar en todo excepto la ventana OOS pedida; test = últimos oos_days
    bars_oos = int(args.oos_days * 6)
    if len(df) <= bars_oos + 200:
        cut = int(len(df) * args.train_frac)
        train, test = df.iloc[:cut].copy(), df.iloc[cut:].copy()
    else:
        train, test = df.iloc[:-bars_oos].copy(), df.iloc[-bars_oos:].copy()

    print("🧠 Training model on train / scoring OOS…")
    model = train_model(train)
    test["prob_up"] = predict_prob_up(model, test)

    # Cargar locks y pesos
    pL = load_lock(LOCK_LONG,  FALLBACK_LONG)
    pS = load_lock(LOCK_SHORT, FALLBACK_SHORT)
    wL_locked, wS_locked = load_weights(default_wL=0.60, default_wS=0.40)
    wL = args.wL if args.wL is not None else wL_locked
    wS = args.wS if args.wS is not None else wS_locked

    # Métricas por pierna (OOS)
    rL = run_portfolio(test.copy(), pL, pS, wL=1.0, wS=0.0)
    rS = run_portfolio(test.copy(), pL, pS, wL=0.0, wS=1.0)

    # Baseline y selección (búsqueda de pesos sencilla bajo MDD cap)
    base = run_portfolio(test.copy(), pL, pS, wL=wL, wS=wS)

    mdd_cap = parse_mdd_cap(args.mdd_cap_mode, rL, rS)

    def search_weights():
        best = None
        grid_L = [1.00,0.85,0.75,0.60,0.50,0.40,0.35,0.30,0.25,0.20,0.15]
        grid_S = [1.00,0.90,0.85,0.75,0.60,0.50,0.40,0.35,0.30,0.25,0.20]
        for wl in grid_L:
            for ws in grid_S:
    ...
    best = search_weights()
    if best is None:
        best = {"wL": wL, "wS": wS, **base, "feasible": False}

    # Cargar último resumen para comparar (si existe)
    prev = None
    ...
    # Decisión de autolock
    improved = False
    within_cap = best["mdd"] <= mdd_cap
    if prev and "selected" in prev:
        prev_score = float(prev["selected"].get("score", 0.0))
        improved = (best["score"] > prev_score) and within_cap
    else:
        # Si no hay previo, solo lockear si está dentro de cap
        improved = within_cap

    if args.autolock and improved:
        save_weights(best["wL"], best["wS"])
        autolock_msg = "✅ Autolock: nuevos pesos guardados."
    else:
        autolock_msg = "↩︎ Autolock omitido (sin mejora de score o MDD > cap, o flag no pasado)."
    ...
    print(f"Baseline   → wL {wL:.2f}, wS {wS:.2f} | Net {base['net']:.2f}, PF {base['pf']:.2f}, WR {base['wr']:.2f}%, Trades {base['trades']}, MDD {base['mdd']:.2f}, Score {base['score']:.2f}")
    ...
    out_json = {
        ...
        "mdd_cap_mode": args.mdd_cap_mode,
        "legs": {"long": rL, "short": rS},
        "baseline": {"wL": wL, "wS": wS, **base},
        "selected": best,
        "autolock": {"enabled": bool(args.autolock), "executed": bool(args.autolock and improved)}
    }
    path_ts = os.path.join(REPORT_DIR, f"conflict_guard_weekly_{stamp}.json")
    with open(path_ts, "w") as f:
        json.dump(out_json, f, indent=2)
    with open(LATEST_SUMMARY, "w") as f:
        json.dump(out_json, f, indent=2)
    print(f"📝 Saved weekly summary → {path_ts}")
    print(f"📌 Updated latest → {LATEST_SUMMARY}")
