...
def load_and_feat():
    ...
    df.columns = getattr(df.columns, 'get_level_values', lambda _: df.columns)(0).str.lower()
...
def main():
    ...
    res   = [ backtest_period(df, d, model, BEST_P) for d in (30,60,90) ]
    print(pd.DataFrame(res).set_index("days"))
