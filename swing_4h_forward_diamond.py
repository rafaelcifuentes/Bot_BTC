...
try:
    from scripts.emit_diamante_bars_hook import emit_bars_if_possible
    _EMIT_BARS_HOOK = True
except Exception as _e:
    _EMIT_BARS_HOOK = False
...
# >>> NUEVO: simulación bar-a-bar para exportar serie por-barra
def _simulate_series(df: pd.DataFrame, probas: pd.DataFrame, p: dict, long_only=False) -> pd.DataFrame:
    """
    Recorre TODA la ventana de test (índices en 'probas') y devuelve DataFrame:
    index=timestamp, col=ret_4h (delta_equity/EQUITY0 por barra). SAFE (sin lookahead) si probas lo son.
    """
    idx = probas.index.intersection(df.index).sort_values()
    if len(idx) == 0:
        return pd.DataFrame(columns=["ret_4h"])
    period = df.loc[idx].copy()
    probas = probas.loc[idx].copy()
    ...
    for ts, r in period.iterrows():
        ...
        rets.append((ts, (equity - prev) / EQUITY0))

    out = pd.DataFrame(rets, columns=["timestamp","ret_4h"]).set_index("timestamp")
    return out
...
def main():
    ...
    # Costes y slippage (Día 4)
    ap.add_argument(
        "--slip",
        type=float,
        default=None,
        help="Slippage por lado (fracción). Si no se pasa, usa SLIP por defecto del script."
    )
    ap.add_argument(
        "--cost",
        type=float,
        default=None,
        help="Comisión ida+vuelta total (fracción). Si no se pasa, usa COST por defecto del script."
    )
    args = ap.parse_args()
    ...
    # --- Modelo de costes (Día 4) ---
    global SLIP, COST
    eff_slip = SLIP if args.slip is None else float(args.slip)
    eff_cost = COST if args.cost is None else float(args.cost)
    SLIP, COST = eff_slip, eff_cost
    print(f"[cost-model] slip={SLIP:.6f}  cost_rt={COST:.6f}")
    ...
    # Buy & Hold (sobre ventana de test)
    cut = int(len(df)*args.split)
    ...
    # >>> NUEVO (opt-in): exportar barras por-barra solo si CORAZON_EXPORT_BARS=1
    if os.getenv("CORAZON_EXPORT_BARS", "0") == "1":
        try:
            Xs, ys = _prepare_xy(df.iloc[:cut], shift_features=True)
            model_safe = train_model_from_xy(Xs, ys)
            probas_all_safe = _predict_probas(model_safe, df, shift_features=True)
            probas_test_safe = probas_all_safe.loc[df.index[cut:]]
            bars_df = _simulate_series(df.loc[probas_test_safe.index], probas_test_safe, params,
                                       long_only=args.long_only)
            out_bars = "reports/diamante_btc_costes_week1_bars.csv" if args.out_csv else "reports/diamante_bars.csv"
            os.makedirs(os.path.dirname(out_bars) or ".", exist_ok=True)
            bars_df.reset_index().to_csv(out_bars, index=False)
            print(f"[OK] Barras (Diamante por-barra) → {out_bars}  (rows={len(bars_df)})")
        except Exception as e:
            print(f"[WARN] Export de barras (serie por-barra) falló: {e}")
    else:
        print("[INFO] Export de barras desactivado (CORAZON_EXPORT_BARS!=1).")
