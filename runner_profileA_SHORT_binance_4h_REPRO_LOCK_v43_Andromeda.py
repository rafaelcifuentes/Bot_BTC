# runner_profileA_SHORT_binance_4h_REPRO_LOCK_v43_Andromeda.py
# 4h swing SHORT — ccxt+fallbacks loader, ML prob_up (invertida para cortos),
# ATR risk, walk-forward + HOLDOUT, stress y autolock con política 1%+MDD
# Repro flags: --primary-only, --freeze-end, --date-split, --repro-lock, --no-autolock
# Reqs: pip install ccxt pandas pandas_ta numpy scikit-learn yfinance
...
# -----------------------
# Config / Parámetros (SHORT)
# -----------------------
SYMBOL = "BTC/USDT"     # símbolo primario ccxt
YF_SYMBOL = "BTC-USD"   # yfinance
TIMEFRAME = "4h"
MAX_BARS = 3000
SINCE_DAYS = 600
MIN_BARS_REQ = 800
...
LOCK_PATH = "./profiles/profileA_short_binance_4h.json"
...
# Últimos params sandbox SHORT (fallback si no existe lock)
PARAMS_FALLBACK = {
    "threshold": 0.46,        # usamos 0.46 para alinear con el long más performante
    "adx4_min": 4,
    "adx1d_min": 0,
    "trend_mode": "slope_down",     # "none" | "slope_down"
    "sl_atr_mul": 1.10,
    "tp1_atr_mul": 1.60,
    "tp2_atr_mul": 4.50,
    "trail_mul": 0.80,
...
# Autolock policy (1% + tie-break por MDD) — misma que el long
...
EX_SYMBOLS_PRIMARY = {"binance": ["BTC/USDT"]}
...
def load_4h_stitched(primary_only: bool) -> Tuple[pd.DataFrame, List[str]]:
    sources, dfs = [], []
    ex_map = EX_SYMBOLS_PRIMARY if primary_only else EX_SYMBOLS_ALL
    ...
    for _, r in data.iterrows():
        if pos is None:
            ...
            if (not pos["p1"]) and (r["low"] <= pos["t1"]):
                ...
                pnl = (pos["e"] - pos["t1"]) * part_sz  # short: entry - exit
            ...
            # trailing stop para short: stop = lm + trail*ATR
...
def split_train_test(df: pd.DataFrame, train_frac: float = 0.7, date_split: Optional[str] = None):
    if date_split:
        ts = pd.to_datetime(date_split).tz_localize(None)
        cut = df.index.searchsorted(ts)
        cut = int(np.clip(cut, 1, len(df)-1))
    else:
...
def walk_forward_short(df: pd.DataFrame, p: Dict, horizon_days: int = 90, folds: int = 6) -> pd.DataFrame:
    rows = []
...
def holdout_eval_short(df: pd.DataFrame, p: Dict, holdout_days: int = HOLDOUT_DAYS) -> Dict:
...
def cost_slip_stress_short(df: pd.DataFrame, p: Dict, cost_mults=(0.5,1.0,1.5), slip_mults=(0.5,1.0,1.5)) -> pd.DataFrame:
    rows = []
    global COST, SLIP
...
def snapshot_params(params: Dict, note: str = "") -> None:
    ensure_dirs()
    blob = params.copy()
    blob["_meta"] = {"profile": "A_SHORT", "version": "v4.3", "note": note, "timestamp_utc": pd.Timestamp.utcnow().isoformat()}
    snap = os.path.join(SNAPSHOT_DIR, f"profileA_short_binance_4h_{now_stamp()}.json")
...
def bump_and_save_params(params: Dict, version: str = "v4.3") -> None:
    ensure_dirs()
    blob = params.copy()
    blob["_meta"] = {"profile":"A_SHORT","version":version,"timestamp_utc": pd.Timestamp.utcnow().isoformat()}
    with open(LOCK_PATH, "w") as f:
        json.dump(blob, f, indent=2)
    snap = os.path.join(SNAPSHOT_DIR, f"profileA_short_binance_4h_{now_stamp()}.json")
    with open(snap, "w") as f:
        json.dump(blob, f, indent=2)
    print(f"🔒 Auto-locked SHORT params → {LOCK_PATH}")
    print(f"📎 Snapshot → {snap}")

# -----------------------
# Micro-grid SHORT (alrededor de params)
# -----------------------
def micro_grid(params: Dict) -> List[Dict]:
    ...
    risks  = sorted(set([round(params["risk_perc"], 4), 0.0045, 0.0050, 0.0055]))
...
# -----------------------
# Competitiveness summary (1–10)
# -----------------------
def _scale_pf(pf: float) -> float:
    return float(np.clip(1 + (pf - 1) / 4 * 9, 1, 10))
...
def _robust_from_stress(base_score: float, worst_score: float) -> float:
    ...
    drop_ratio = max(0.0, (base_score - worst_score) / base_score)
...
def save_competitiveness_summary(stamp: str,
                                 ...
                                 pick90: Optional[Dict], pick180: Optional[Dict],
                                 short_tag: str = "short") -> str:
    s90, s180 = (pick90 or base90), (pick180 or base180)
    worst_row = None
    if stress_df is not None and not stress_df.empty:
        worst_row = stress_df.sort_values("score180").iloc[0]
    worst_score_180 = float(worst_row["score180"]) if worst_row is not None else base180.get("score", 0.0)

    ratings = {
        "90d_slice": {
            "pf": s90.get("pf", 0.0), "mdd": s90.get("mdd", 0.0), "win_rate": s90.get("win_rate", 0.0),
            "scores": {
                "pf_score": round(_scale_pf(s90.get("pf", 0.0)), 1),
                "mdd_control": round(_scale_mdd(s90.get("mdd", 0.0)), 1),
                "consistency": 8.0,
                "robustness": round(_robust_from_stress(s180.get("score", 0.0), worst_score_180), 1),
                "trade_efficiency": round(_eff_from_wr(s90.get("win_rate", 0.0)), 1)
            }
        },
        "180d_slice": {
            "pf": s180.get("pf", 0.0), "mdd": s180.get("mdd", 0.0), "win_rate": s180.get("win_rate", 0.0),
            "scores": {
        ...
        "holdout": {
            "pf": hold.get("pf", 0.0), "mdd": hold.get("mdd", 0.0), "win_rate": hold.get("win_rate", 0.0),
            "scores": {
    ...
    overall = {
        ...
        "note": "Escalado heurístico 1–10 para comparar entre tus propias corridas."
    }

    ensure_dirs()
    out = {
        "stamp_utc": pd.Timestamp.utcnow().isoformat(),
        "headline": "Competitiveness (SHORT) 1–10",
        "ratings": ratings,
        "overall": overall
    }
    path = os.path.join(REPORT_DIR, f"competitiveness_summary_{short_tag}_{stamp}.json")
...
def save_report_with_stamp(stamp: str,
                           ...
                           pick_params: Optional[Dict], pick90: Optional[Dict], pick180: Optional[Dict],
                           short_tag: str = "short") -> Tuple[str, str]:
    ensure_dirs()
    wf_csv = os.path.join(REPORT_DIR, f"walkforward_{short_tag}_{stamp}.csv")
    json_path = os.path.join(REPORT_DIR, f"summary_{short_tag}_{stamp}.json")
...
# -----------------------
def main():
    parser = argparse.ArgumentParser(description="Profile A SHORT 4h — REPRO LOCK v4.3 (Andromeda)")
    parser.add_argument("--primary-only", action="store_true", help="Usar solo Binance (mejor reproducibilidad)")
    parser.add_argument("--freeze-end", type=str, default=None, help="Cortar datos hasta esta fecha (YYYY-MM-DD[THH:MM])")
    parser.add_argument("--date-split", type=str, default=None, help="Fecha de corte train/test (YYYY-MM-DD)")
    parser.add_argument("--repro-lock", action="store_true", help="No modificar lock salvo autolock")
    parser.add_argument("--no-autolock", action="store_true", help="Desactivar autolock")
    args = parser.parse_args()

    print("🔄 Downloading 4h data (ccxt/Binance + fallbacks)…")
    print(f"Flags → primary_only={args.primary_only}, freeze_end={args.freeze_end}, date_split={args.date_split}, repro_lock={args.repro_lock}")
    ...
    # freeze_end (repro)
    if args.freeze_end:
        t_end = pd.to_datetime(args.freeze_end).tz_localize(None)
        df_raw = df_raw.loc[:t_end]

    params = load_locked_params()
    snapshot_params(params, note="pre-run snapshot (SHORT)")
    ...
    # 70/30 o fecha fija
    train, test = split_train_test(df, train_frac=0.7, date_split=args.date_split)
    train_with, test_with = attach_probs(train, test)

    # Baseline (SHORT)
    print("\n— Baseline SHORT (70% train → OOS test) —")
    print(f"Using short params: {params}")
    base90  = backtest_short(test_with, params, days=90)
    base180 = backtest_short(test_with, params, days=180)
    fmt_row("90d  base (short)", base90)
    fmt_row("180d base (short)", base180)

    # Walk-forward
    print("\n— Walk-forward 6×90 (rolling, non-overlapping) —")
    wf = walk_forward_short(df, params, horizon_days=90, folds=6)
    if not wf.empty:
    ...
    hold = holdout_eval_short(df, params, HOLDOUT_DAYS)
    fmt_row(f"Holdout {HOLDOUT_DAYS}d (short)", hold)

    # Stress (SHORT)
    print("\n— Cost/slippage stress (short) —")
    stress_base = cost_slip_stress_short(test_with, params)
    if not stress_base.empty:
        print(stress_base.to_string(index=False))

    # Micro-grid (omitimos si repro_lock estricto? mejor lo dejamos; autolock respeta flags)
    print("\n— Micro-grid (around short params) —")
    ...
    for i, c in enumerate(candidates):
        r90  = backtest_short(test_with, c, days=90)
        r180 = backtest_short(test_with, c, days=180)
        scored.append({
            "score": r180["score"],
            "mdd_neg": -r180["mdd"],
            "net": r180["net"],
    ...
    if scored:
        ...
        print("Picked SHORT params (subset):", {k: best_pick[k] for k in ("threshold","adx4_min","sl_atr_mul","risk_perc","trend_mode")})
        fmt_row("90d  pick (short)", best90)
        fmt_row("180d pick (short)", best180)
    else:
        print("No candidates generated.")
    stress_pick = cost_slip_stress_short(test_with, best_pick) if best_pick is not None else pd.DataFrame()

    # Guardado de reportes + competitiveness
    stamp = now_stamp()
    _json_path, _csv_path = save_report_with_stamp(stamp, sources, base90, base180, wf, wf_agg, hold, params,
                                                   stress_base, stress_pick, best_pick, best90, best180, short_tag="short")
    save_competitiveness_summary(stamp, base90, base180, wf_agg, hold, stress_base, best90, best180, short_tag="short")

    # Autolock
    if not args.no_autolock and (best_pick is not None) and AUTOLOCK_ON_IMPROVE:
        ok, info = should_autolock(base180, best180, MDD_TOL_MULT)
        if ok and not args.repro_lock:
            bump_and_save_params(best_pick, version="v4.3")
        else:
            print("↩︎ Autolock skipped", info, "(repro_lock on)" if args.repro_lock else "")
    else:
        print("↩︎ Autolock skipped (disabled or no candidates).")
