# Progreso — Histórico consolidado

> Este archivo guarda el **histórico completo**. El archivo principal `Progreso.md` es *append-only* y solo añade bloques nuevos con fecha.

## V1.0 — Histórico recuperado (pegar aquí)
<!-- [restaurar:V1.0] pega aquí el texto recuperado -->

## V2.0 — Histórico recuperado (pegar aquí)
<!-- [restaurar:V2.0] pega aquí el texto recuperado -->
...
# versión de un commit/fecha específica
...
## Recuperación rápida (paso a paso)

### A) Con Git (preferido)
1. Listar commits que tocaron `Progreso.md` (sigue renombres):
   ```bash
   git log --follow --oneline --decorate -- docs/mini_accum/Progreso.md
   ```
2. Tomar la versión anterior a la edición que borró/recortó y guardarla:
   ```bash
   # el commit inmediatamente anterior que tocó el archivo
   PREV=$(git log --follow --pretty=format:%H --skip=1 -n1 -- docs/mini_accum/Progreso.md)
   git show "$PREV":docs/mini_accum/Progreso.md > /tmp/Progreso_prev.md
   ```
3. Volcar el contenido previo completo al histórico (sin overwrites de lo actual):
   ```bash
   mkdir -p docs/mini_accum
   cat /tmp/Progreso_prev.md >> docs/mini_accum/progreso_HISTORICO.md
   echo -e "\n\n> Fuente: commit $PREV\n" >> docs/mini_accum/progreso_HISTORICO.md
   ```
4. (Opcional) Enlazar desde `Progreso.md` donde veas `[restaurar:V1.0]` o `[restaurar:V2.0]`:
   - Reemplaza el marcador por un enlace:
     ```markdown
     [Ver histórico V1.0](docs/mini_accum/progreso_HISTORICO.md#v10--histórico-recuperado)
     ```

### B) Con PyCharm Local History
1. Click derecho sobre `docs/mini_accum/Progreso.md` → **Local History** → **Show History**.
2. Selecciona un snapshot *previo* a la edición y copia el contenido V1.0/V2.0.
3. Péga ese contenido en:
   - `docs/mini_accum/progreso_HISTORICO.md` bajo el bloque correspondiente (**V1.0** o **V2.0**).
   - O bien sustituye los marcadores `<!-- [restaurar:...] -->` con el texto recuperado en este mismo archivo histórico.
4. Guarda y commitea los cambios.

### C) Verificación rápida
```bash
rg -n "V1\.0|V2\.0|\[restaurar" docs/mini_accum
```
Si no quedan marcadores `[restaurar]`, la restauración está completa.#!/bin/bash
...
> Fuente: commit 754f94216864433a39d88eb27cd41a2e0a2f735e
